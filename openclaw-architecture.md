...
**Pin upstream (tham chiếu khi đọc tài liệu này):** release `v2026.6.6` — commit `8c802aa683` trên `openclaw/openclaw`, đồng bộ với nhánh `upstream/pin` trong repo vendored `openclaw/openclaw` và image `alpine/openclaw:2026.6.6` (OCI revision `de16f57dea`).

### 4.2 Các Endpoint HTTP

Cùng port với Gateway (HTTP multiplex với WebSocket). Danh sách dưới đây khớp dispatcher trong `openclaw/src/gateway/server-http.ts` và tài liệu `docs/gateway/` (có thể thay đổi theo bản fork).
...
| **Khác (CHANGELOG, không đổi §4.2)** | Ví dụ: metadata reload config cho tool (`config` lookup), tối ưu startup gateway song song kênh, CRON queue Matrix/Mattermost/Google Chat, nhiều sửa agent/cron/channel — chi tiết xem `openclaw/CHANGELOG.md`. |

### 4.9 Bề Mặt Mạng Gateway — HTTP, WebSocket và Bật/Tắt Mặc Định

> Nguồn: `openclaw/src/gateway/server-http.ts`, `server-runtime-config.ts`, `src/gateway/security-path.ts`, `docs/gateway/index.md`, `docs/gateway/openai-http-api.md`, `docs/gateway/tools-invoke-http-api.md`, `docs/plugins/admin-http-rpc.md`, `docs/gateway/protocol.md`.
...
> **Nguồn (sync từ `openclaw/openclaw`):** `src/agents/tool-catalog.ts` (catalog + profile defaults), `src/agents/openclaw-tools.ts` (runtime assembly), `src/agents/tool-policy-shared.ts` (`group:*` expansion). Gateway Docker hiện dùng image `alpine/openclaw` (ví dụ **2026.6.9**); repo local có thể lệch patch (ví dụ `2026.6.6`).
...
> **Nguồn:** `openclaw/openclaw` tag `v2026.6.5` — `docs/tools/exec.md`, `docs/tools/exec-approvals.md`, `docs/tools/exec-approvals-advanced.md`, schema `runtime-schema`.
...
**Tham chiếu code:** `packages/workspace-sync/src/openclaw-config-merge.ts` → `mergeExecToolsIntoConfig()`.
...
**Mã nguồn:** `openclaw/src/agents/skills/workspace.ts` (`loadWorkspaceSkillEntries`, `buildWorkspaceSkillsPrompt`, watcher `refresh.ts`).
...
Tham chiếu đầy đủ: `openclaw/docs/tools/creating-skills.md`.
...
Schema: `openclaw/docs/tools/skills-config.md`.

#### 11.5.7 ClawHub & CLI

- Registry: [clawhub.ai](https://clawhub.ai)
- CLI: `openclaw skills search | install | update | list | info | check` — `openclaw/docs/cli/skills.md`
...
> **Nguồn:** `openclaw/src/agents/sandbox/*`, `src/config/types.sandbox.ts`, `src/config/types.agents-shared.ts`, `docs/install/docker.md` (ghi chú sandbox). Cập nhật: 2026-06-08.
...
**Source chính (openclaw/openclaw):**
...
## 31. Cấu Trúc Mã Nguồn `openclaw/openclaw` — Kết Nối & Phân Tầng

> Phạm vi: **chỉ** repo `openclaw/` — đây là bản OpenClaw (runtime Gateway). Không mô tả các repo khác trong monorepo SaaS. Nguồn: `src/`, `ui/`, `extensions/`, `apps/`, `README.md`.

### 31.1 OpenClaw Là Gì Trong Repo Này?

`openclaw/openclaw` chạy **một process Gateway** (`openclaw gateway run`) — control plane trung tâm của OpenClaw:
...
flowchart TB
    ...
    subgraph GATEWAY["openclaw/openclaw — src/gateway/"]
        HTTP["server-http.ts\nHTTP dispatcher"]
...
|-----|---------|-----------|-------------|
| **UI (hiển thị)** | `openclaw/ui/` | **Lit** + **Vite** | Render Control UI; **không** chứa business logic gateway |
...
| **Gateway (control plane)** | `openclaw/src/gateway/` | Node.js | HTTP + WS server, auth, routing RPC |
...
Tất cả đều nằm trong hoặc gọi vào `openclaw/openclaw` — **không** có repo UI riêng ngoài `ui/`:
...
| **UI ở đâu**? | `openclaw/ui/` — Lit/Vite; serve static qua Gateway; logic hiển thị + `controllers/` gọi WS. |
