...
> **Phạm vi:** Chỉ **`openclaw-fork/`** (runtime Gateway upstream). **Không** mô tả control plane Claw Dashboard (API, DB, sync volume, chat proxy) — xem **`workflow.md`** (repo root).
>
...
**Pin upstream (tham chiếu khi đọc tài liệu này):** release `v2026.6.6` — commit `8c802aa683` trên `openclaw/openclaw`, đồng bộ với nhánh `upstream/pin` trong repo vendored `openclaw-fork` và image `alpine/openclaw:2026.6.6` (OCI revision `de16f57dea`).

### 4.2 Các Endpoint HTTP

Cùng port với Gateway (HTTP multiplex với WebSocket). Danh sách dưới đây khớp dispatcher trong `openclaw-fork/src/gateway/server-http.ts` và tài liệu `docs/gateway/` (có thể thay đổi theo bản fork).
...
| **Khác (CHANGELOG, không đổi §4.2)** | Ví dụ: metadata reload config cho tool (`config` lookup), tối ưu startup gateway song song kênh, CRON queue Matrix/Mattermost/Google Chat, nhiều sửa agent/cron/channel — chi tiết xem `openclaw-fork/CHANGELOG.md`. |

### 4.9 Bề Mặt Mạng Gateway — HTTP, WebSocket và Bật/Tắt Mặc Định

> Nguồn: `openclaw-fork/src/gateway/server-http.ts`, `server-runtime-config.ts`, `src/gateway/security-path.ts`, `docs/gateway/index.md`, `docs/gateway/openai-http-api.md`, `docs/gateway/tools-invoke-http-api.md`, `docs/plugins/admin-http-rpc.md`, `docs/gateway/protocol.md`.
...
> **Nguồn (sync từ `openclaw-fork`):** `src/agents/tool-catalog.ts` (catalog + profile defaults), `src/agents/openclaw-tools.ts` (runtime assembly), `src/agents/tool-policy-shared.ts` (`group:*` expansion). Gateway Docker hiện dùng image `alpine/openclaw` (ví dụ **2026.6.9**); repo local có thể lệch patch (ví dụ `2026.6.6`).
...
**Config tối thiểu (DuckDuckGo — Claw Dashboard OSS default sau sync):**
...
> **Nguồn:** `openclaw-fork` tag `v2026.6.5` — `docs/tools/exec.md`, `docs/tools/exec-approvals.md`, `docs/tools/exec-approvals-advanced.md`, schema `runtime-schema`.
...
3. **Sandbox (Docker) trong Claw Dashboard UI?**  
...
#### Claw Dashboard sync (`workspace-sync`) — hành vi hiện tại
...
Hệ quả: nếu Agent A `ask: always` và Agent B `ask: off`, sau sync chỉ còn **một** `tools.exec` (từ agent default). Muốn policy khác nhau → cần `agents.list[].tools.profile` + deny `exec` ở agent hạn chế, hoặc mở rộng compiler Claw Dashboard sau.

**Tham chiếu code:** `studio/packages/workspace-sync/src/openclaw-config-merge.ts` → `mergeExecToolsIntoConfig()`.
...
> Phần dưới đối chiếu `openclaw-fork/docs/tools/skills.md`, `creating-skills.md`, `skills-config.md`, `docs/cli/skills.md` và mã `src/agents/skills/*`, `src/agents/skills-install*.ts`, `src/gateway/server-methods/skills*.ts` (pin `v2026.6.6` / `8c802aa683`). Cách Claw Dashboard sync skill từ DB → volume: `workflow.md`.
...
**Mã nguồn:** `openclaw-fork/src/agents/skills/workspace.ts` (`loadWorkspaceSkillEntries`, `buildWorkspaceSkillsPrompt`, watcher `refresh.ts`).
...
Tham chiếu đầy đủ: `openclaw-fork/docs/tools/creating-skills.md`.
...
Schema: `openclaw-fork/docs/tools/skills-config.md`.

#### 11.5.7 ClawHub & CLI

- Registry: [clawhub.ai](https://clawhub.ai)
- CLI: `openclaw skills search | install | update | list | info | check` — `openclaw-fork/docs/cli/skills.md`
...
> **Nguồn:** `openclaw-fork/src/agents/sandbox/*`, `src/config/types.sandbox.ts`, `src/config/types.agents-shared.ts`, `docs/install/docker.md` (ghi chú sandbox). Cập nhật: 2026-06-08.
...
**Lưu ý triển khai OSS (Claw Dashboard):** gateway container mount `OPENCLAW_STATE_DIR` = `data/projects/{projectId}/`. Sandbox Docker backend cần gateway **truy cập Docker socket** để spawn `openclaw-sbx-*` — kiểm tra compose/runtime trước khi bật `sandbox.mode` khác `"off"` trên stack hosted.
...
sequenceDiagram
    ...
    Prompt->>Prompt: Build system prompt + bootstrap files<br/>(xem §22.5; daily memory chỉ turn đầu session)
...
|------|---------|---------|
| `AGENTS.md` | Operating rules, công việc, hành vi | **Quan trọng nhất**; inject khi bootstrap eligible |
| `SOUL.md` | Personality, tone, brevity, humor | Inject khi bootstrap eligible (không phải lúc nào cũng mỗi turn — xem §22.5.3) |
...
**Giới hạn:**
- Per-file: `agents.defaults.bootstrapMaxChars` (mặc định **20,000** chars)
- Tổng tất cả files: `agents.defaults.bootstrapTotalMaxChars` (mặc định **60,000** chars)
- Quá giới hạn → tự động truncate + cảnh báo

> Chi tiết **khi nào** các file này được inject (mỗi turn vs turn đầu vs bỏ qua) → xem **§22.5**.
...
### 22.5 Per-Turn Context Assembly — Mỗi Lượt Chat Load Gì?

> ℹ️ **[SYNTHESIS — Diagram tôi tự vẽ]** Luồng dưới đây tổng hợp từ `embedded-agent-runner/run/attempt.ts`, `bootstrap-files.ts`, `system-prompt.ts`, `auto-reply/reply/startup-context.ts`. OpenClaw docs mô tả từng phần rời rạc; diagram này gom lại theo **một lượt chat**.

Mỗi lần user gửi tin nhắn, Gateway không chỉ forward text — nó **assemble** một request LLM gồm ba lớp độc lập:

```mermaid
sequenceDiagram
    participant User as User message
    participant GW as Agent Loop
    participant WS as Workspace (disk)
    participant JSONL as Session transcript
    participant LLM as Model

    User->>GW: Tin nhắn mới (+ media nếu có)
    GW->>JSONL: Đọc toàn bộ history session
    GW->>WS: Đọc bootstrap files (nếu eligible)
    GW->>GW: Build system prompt + skills catalog
    Note over GW,WS: Daily memory (memory/*.md)<br/>chỉ prepend turn đầu session
    GW->>LLM: system + messages[] + user turn
    LLM-->>GW: Assistant (+ tool calls)
    GW->>JSONL: Append transcript
```

#### 22.5.1 Ba lớp context gửi tới model

| Lớp | Nguồn | Mỗi turn? | Vai trò |
|-----|-------|-----------|---------|
| **System prompt** | Runtime build | **Có** (rebuild) | Tooling, safety, skills catalog, workspace bootstrap, runtime metadata |
| **Messages array** | `sessions/<id>.jsonl` | **Có** (cộng dồn) | Toàn bộ user/assistant/tool history; compaction summary nếu đã compact |
| **User turn hiện tại** | Channel inbound | **Có** | Text + attachment; startup prelude chỉ turn đầu session |

#### 22.5.2 System prompt — thành phần cố định mỗi turn

| Thành phần | Inject vào prompt? | Ghi chú |
|------------|-------------------|---------|
| Tooling + execution bias + safety | Có | Danh sách tool **sau policy filter** (`tools.allow` / profile) |
| Skills catalog (`<available_skills>` XML) | Có (mode `full`) | Chỉ **metadata** (tên + path); **không** gửi full `SKILL.md` |
| Workspace path + docs links | Có | |
| Runtime block (OS, model, timezone, channel) | Có | |
| Memory guidance (`memory_search` / `memory_get`) | Có (legacy engine) | Hướng dẫn dùng tool, không phải nội dung memory |
| **Bootstrap workspace files** | Tùy `contextInjection` | Xem §22.5.3 |
| HEARTBEAT / sandbox / reply tags | Tùy config | |

Source: `embedded-agent-runner/system-prompt.ts` → `buildAgentSystemPrompt()`; bootstrap files chèn dưới heading `# Project Context`.

#### 22.5.3 Bootstrap workspace files — đọc từ disk

**Thứ tự đọc** (source: `agents/workspace.ts` → `loadWorkspaceBootstrapFiles()`):

```
AGENTS.md → SOUL.md → TOOLS.md → IDENTITY.md → USER.md
→ HEARTBEAT.md → BOOTSTRAP.md → MEMORY.md (chỉ khi file tồn tại)
```

**Lọc trước khi inject:**

| Điều kiện | Hành vi |
|-----------|---------|
| File không tồn tại | Bỏ qua (trừ `AGENTS.md` / `TOOLS.md` vẫn có entry `missing`) |
| Workspace setup đã xong | Loại `BOOTSTRAP.md` khỏi inject |
| Heartbeat tắt (default agent) | Loại `HEARTBEAT.md` |
| Sub-agent session | Chỉ `AGENTS.md` + `TOOLS.md` |
| Cron session | Subset: `AGENTS`, `TOOLS`, `SOUL`, `IDENTITY`, `USER` (không `BOOTSTRAP`, `MEMORY`) |
| Heartbeat run (`lightweight`) | Chỉ `HEARTBEAT.md` |
| Hooks `before_prompt_build` | Có thể thêm/sửa bootstrap files |

**`agents.defaults.contextInjection`** — quyết định inject bootstrap **mỗi turn hay không**:

| Mode | Mặc định | Hành vi |
|------|----------|---------|
| `always` | **Có** | Đọc + inject bootstrap files **mỗi turn** |
| `continuation-skip` | | Turn đầu inject đủ; các turn sau **bỏ qua** nếu transcript đã có marker `openclaw:bootstrap-context:full` và chưa bị compaction sau marker |
| `never` | | Không inject bootstrap |

Source: `bootstrap-files.ts` → `resolveContextInjectionMode()`, `resolveAttemptBootstrapContext()` trong `attempt.context-engine-helpers.ts`.

**Bootstrap mode** (`full` / `limited` / `none`) — chỉ khi workspace còn pending onboarding (`BOOTSTRAP.md`):

| Mode | Khi nào | Inject |
|------|---------|--------|
| `full` | User-facing, primary run, có tool `read` | Bootstrap files vào system prompt + hướng dẫn hoàn tất onboarding |
| `limited` | Non-canonical workspace hoặc không có file access | Hướng dẫn rút gọn, không claim bootstrap xong |
| `none` | Setup xong / heartbeat / cron / non-interactive | Không inject bootstrap content |

Source: `bootstrap-mode.ts` → `resolveBootstrapMode()`.

**Cache:** `bootstrap-cache.ts` giữ snapshot per `sessionKey`; **mỗi turn vẫn re-read disk** nhưng reuse snapshot nếu nội dung file không đổi — session dài vẫn thấy edit workspace mới.

#### 22.5.4 Startup context — daily memory (không nằm trong system prompt)

`memory/YYYY-MM-DD.md` **không** đi qua bootstrap injector. Chúng được load bởi `buildSessionStartupContextPrelude()` và **prepend vào user message** của **turn đầu session mới / reset** — không phải mỗi tin nhắn.

| Config | Mặc định | Ý nghĩa |
|--------|----------|---------|
| `agents.defaults.startupContext.enabled` | `true` | Bật startup prelude |
| `startupContext.applyOn` | `new`, `reset` | Chỉ session mới hoặc sau `/reset` |
| `startupContext.dailyMemoryDays` | **2** | Hôm nay + hôm qua (theo user timezone) |
| `startupContext.maxFileChars` | 1,200 | Truncate per file |
| `startupContext.maxTotalChars` | 2,800 | Truncate tổng prelude |

Prelude có banner `[Startup context loaded by runtime]` và cảnh báo treat daily memory là **untrusted notes** — tách biệt với `MEMORY.md` trong bootstrap.

Source: `auto-reply/reply/startup-context.ts`; gọi từ `get-reply-run.ts` khi `shouldApplyStartupContext()`.

#### 22.5.5 Conversation history (messages array)

Mỗi turn load **toàn bộ transcript** từ `data/projects/<project>/agents/<agent>/sessions/<sessionId>.jsonl`:

- User / assistant messages
- Tool calls + tool results
- Compaction records + summary (sau auto/manual `/compact`)
- Custom markers (vd. bootstrap completion)

Đây thường là phần **tốn context nhất** theo thời gian — không phải workspace files.

#### 22.5.6 Những gì KHÔNG gửi mỗi turn

| Không gửi | Lý do |
|-----------|-------|
| Full nội dung `SKILL.md` | Agent đọc on-demand qua tool `read` khi skill match |
| `openclaw.json`, `auth-profiles.json` | Config runtime; provider auth resolve server-side |
| Toàn bộ workspace tree | Chỉ bootstrap allowlist + history |
| `memory/YYYY-MM-DD.md` (từ turn 2+) | Chỉ startup prelude turn đầu session |
| `DREAMS.md` | Không auto-load |
| Arbitrary project files | Chỉ khi agent gọi `read` / `exec` |

#### 22.5.7 Ma trận tóm tắt theo loại session

| Loại session | Bootstrap inject | Daily memory prelude | History |
|--------------|------------------|----------------------|---------|
| Chat thường, turn 1 (`contextInjection: always`) | Đủ files eligible | Có (nếu `startupContext` bật) | Rỗng / ngắn |
| Chat thường, turn 2+ (`always`) | Re-read mỗi turn | Không | Đầy đủ |
| Chat thường, turn 2+ (`continuation-skip`) | **Bỏ qua** | Không | Đầy đủ |
| Sub-agent | `AGENTS.md` + `TOOLS.md` | Không | Riêng |
| Cron | Subset (không `BOOTSTRAP`, `MEMORY`) | Không | Riêng |
| Heartbeat | `HEARTBEAT.md` only (lightweight) | Không | Riêng |

**Source chính (openclaw-fork):**

| File | Vai trò |
|------|---------|
| `src/agents/embedded-agent-runner/run/attempt.ts` | Orchestrate mỗi turn: bootstrap → system prompt → LLM |
| `src/agents/bootstrap-files.ts` | Resolve + filter bootstrap files |
| `src/agents/bootstrap-cache.ts` | Per-session snapshot cache |
| `src/agents/workspace.ts` | `loadWorkspaceBootstrapFiles()` |
| `src/agents/system-prompt.ts` | Render system prompt + `# Project Context` |
| `src/auto-reply/reply/startup-context.ts` | Daily memory prelude |
| `src/skills/loading/workspace.ts` | Skills catalog XML |
...
| File | Load khi | Cách load | Mục đích |
|------|---------|-----------|---------|
| `MEMORY.md` | Mỗi turn (khi bootstrap inject) | System prompt → `# Project Context` (§22.5.3) | Long-term facts, preferences, decisions |
| `memory/YYYY-MM-DD.md` | **Turn đầu** session mới/reset | Prepend user message qua startup prelude (§22.5.4); mặc định 2 ngày | Running context, daily observations |
| `DREAMS.md` | Không auto-load | — | Dream diary để người dùng review |

> **Phân biệt quan trọng:** `MEMORY.md` đi vào **system prompt** (bootstrap). `memory/*.md` đi vào **user turn đầu** — hai đường khác nhau. Agent vẫn có thể đọc thêm memory bất kỳ lúc nào qua `memory_search` / `memory_get`.
...
## 31. Cấu Trúc Mã Nguồn `openclaw-fork` — Kết Nối & Phân Tầng

> Phạm vi: **chỉ** repo `openclaw-fork/` — đây là bản OpenClaw (runtime Gateway). Không mô tả các repo khác trong monorepo SaaS. Nguồn: `src/`, `ui/`, `extensions/`, `apps/`, `README.md`.

### 31.1 OpenClaw Là Gì Trong Repo Này?

`openclaw-fork` chạy **một process Gateway** (`openclaw gateway run`) — control plane trung tâm của OpenClaw:
...
flowchart TB
    ...
    subgraph GATEWAY["openclaw-fork — src/gateway/"]
        HTTP["server-http.ts\nHTTP dispatcher"]
...
|-----|---------|-----------|-------------|
| **UI (hiển thị)** | `openclaw-fork/ui/` | **Lit** + **Vite** | Render Control UI; **không** chứa business logic gateway |
...
| **Gateway (control plane)** | `openclaw-fork/src/gateway/` | Node.js | HTTP + WS server, auth, routing RPC |
...
Tất cả đều nằm trong hoặc gọi vào `openclaw-fork` — **không** có repo UI riêng ngoài `ui/`:
...
| **UI ở đâu**? | `openclaw-fork/ui/` — Lit/Vite; serve static qua Gateway; logic hiển thị + `controllers/` gọi WS. |
...
_Tài liệu được tổng hợp từ `openclaw-fork/` (`README.md`, `VISION.md`, `docs/gateway/configuration.md`, `src/gateway/config-reload-plan.ts`, …). **SSOT worker:** `openclaw-architecture.md` (repo root). **SSOT tính năng Claw Dashboard:** `workflow.md`. Cập nhật: 2026-05-25._
