...
> Áp dụng cho `apps/web/`. Mọi code (kể cả AI) phải tuân thủ.  
> Monorepo: đọc thêm `repo rootAGENTS.md` khi task chạm nhiều app/package.  
...
- **`repo rootapps` storage = local** (avatar PG, chat disk) — không phụ thuộc `RUNTIME_MODE` (API: `apps/api/.agent/rule.md` §1.13).
- **`repo rootapps` là OSS thuần** — không còn `NEXT_PUBLIC_RUNTIME_MODE` hay UI cloud (spawn container, subdomain URL).

### 6.2 Trong phạm vi

`apps/web/` — `app/`, `components/`, `hooks/`, `utils/`, `lib/`, `schemas/`, `stores/`, `public/`.
