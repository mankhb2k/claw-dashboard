...
export function CardIntegrations({ agentId }: CardIntegrationsProps) {
  ...
  const snippets: Record<SnippetTab, string> = {
    curl: `curl -X POST https://api.claw-dashboard.com/v1/chat/completions \\
  -H "Authorization: Bearer ${activeToken}" \\
  -H "Content-Type: application/json" \\
  -d '{"model":"${agentId}","messages":[{"role":"user","content":"Hello!"}]}'`,
    node: `const res = await fetch("https://api.claw-dashboard.com/v1/chat/completions", {
  method: "POST",
  headers: {
...
response = requests.post(
    "https://api.claw-dashboard.com/v1/chat/completions",
