# Ghi chú tiến độ & nợ kỹ thuật — Claw Dashboard API
...
pnpm --filter @claw-dashboard/api lint:ci       # CI parity
pnpm --filter @claw-dashboard/api build
pnpm --filter @claw-dashboard/api test          # chưa gate CI
```
...
| `no-restricted-imports` dto/lib/controller/core + `@claw-dashboard-cloud/*` | ✅ error |
| `UsageSource` / `UsageStatus` → `@claw-dashboard/shared` | ✅ |
...
|------|---------|---------|
| `@typescript-eslint/switch-exhaustiveness-check` | cao | Enum Prisma / `@claw-dashboard/shared` |
