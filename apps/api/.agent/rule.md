...
### 1.13 Self-host — lưu trữ local (`repo rootapps`)

> Repo này là **OSS thuần**: storage local + shared gateway `:18789`.

| Thành phần | `repo rootapps` |
| ---------- | ------------- |
...
> **`@claw-dashboard-cloud/*`:** **không** dùng trong `repo rootapps` — chỉ repo `../cloud/` (§1.13).
...
> **Chính sách cố định:** Claw Dashboard có **một** đường đưa foundation provider API key xuống OpenClaw gateway — block `env` trong `{projectId}/openclaw.json`. **Không** dùng cơ chế song song.
...
| Cách                                                                                     | Lý do cấm                                                                  |
| ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `agents/{agentId}/agent/auth-profiles.json`                                              | Trùng nguồn sự thật; OpenClaw CLI/onboard path — **không** sync từ Claw Dashboard |
...
**Lưu ý OpenClaw worker:** `resolveApiKeyForProvider` vẫn _có thể_ đọc auth profile nếu file tồn tại (thứ tự profile-first). Claw Dashboard **không tạo/không ghi** `auth-profiles.json` — chỉ dựa vào `openclaw.json` `env` sau khi gateway load config.
...
```text
apps/api/src/
packages/control-plane-core/   # nếu extract logic thuần
packages/workspace-sync/     # nếu chạm sync file
packages/database/           # nếu chạm schema
```
...
```text
Task: [một câu]
Scope: apps/api/src/features/projects/<domain>/
