...
jest.mock('../chat-attachments/chat-attachments.service', () => ({
...
function createService() {
  ...
  return { service, workspace, attachments, usageRecorder };
}
...
describe('ChatGatewayProxyService', () => {
  ...
  describe('bridge', () => {
    ...
    it('forwards upstream messages to client', async () => {
      ...
      expect(usageRecorder.tapGatewayFrame).toHaveBeenCalled();
    });

    it('tracks chat.send idempotency and taps final chat events', async () => {
      const { service, usageRecorder } = createService();
      const client = new MockSocket();
      const upstream = new MockSocket();
      openGatewayUpstreamMock.mockResolvedValue(
      ...
      const tapCalls = usageRecorder.tapGatewayFrame.mock.calls;
      const lastTap = tapCalls[tapCalls.length - 1]?.[0] as {
        pendingRuns: Map<string, { idempotencyKey: string }>;
      };
      expect(lastTap.pendingRuns.has('run-chat-1')).toBe(true);
    });
  });
});
