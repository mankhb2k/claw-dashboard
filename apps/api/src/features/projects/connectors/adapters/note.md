...
| Catalog kind enum | `@claw-dashboard/shared` (`CONNECTOR_KINDS`) |
...
Secrets merge vào config qua `mergeConnectorsIntoConfig` (`@claw-dashboard/workspace-sync`) — key names phải khớp MCP server doc (thống nhất với `secretKeys`).
...
Không bắt buộc move catalog skeleton sang `@claw-dashboard/shared` trong phase đầu; registry API đủ cho connector đã implement.
