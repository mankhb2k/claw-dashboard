...
import type { ToolDefinition, ToolResult, ToolContext } from '../lib/types.js';
...
export async function handleTool(
  ...
  switch (toolName) {
    ...
    case "updateCalendarEvent": {
      ...
      const eventResource = buildCalendarEventUpdate(existing as Record<string, unknown>, parsed);

      const response = await ctx.getCalendar().events.update({
        calendarId: parsed.calendarId || 'primary',
        eventId: parsed.eventId,
        requestBody: eventResource as calendar_v3.Schema$Event,
        sendUpdates: parsed.sendUpdates,
...
export const calendarToolSchemas: Record<string, z.ZodTypeAny> = {
  listCalendars: ListCalendarsSchema,
  getCalendarEvents: GetCalendarEventsSchema,
  getCalendarEvent: GetCalendarEventSchema,
