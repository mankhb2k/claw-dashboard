...
- OpenClaw inbound: `chat.send` → `parseMessageWithAttachments` → `media/inbound/` (`openclaw/src/gateway/chat-attachments.ts`, `src/media/store.ts`)
...
> **Đã đối chiếu** `openclaw/openclaw` (2026-06-08). Schema RPC: `ChatSendParamsSchema` — `attachments?: Array<Unknown>`; mỗi item sau normalize phải có **`content` base64** + `mimeType` + `fileName` (`chat-attachments.ts`, `attachment-normalize.ts`). **Không hỗ trợ path-only.**
...
| **D. API đọc disk → base64 → `chat.send`** | ✅ **Khuyến nghị Phase 3** | Không sửa `openclaw/openclaw`; proxy hiện tại forward transparent |
...
| Path | Ghi chú |
|------|---------|
| `apps/web/.../MessageBox/` | Composer UI, drag-drop, paste |
| `apps/web/.../MessageBox/composer-attachments.ts` | Validate client-side |
| `apps/web/lib/api/multipart-upload.ts` | Pattern upload HTTP |
| `apps/api/src/core/users/avatar/` | Pattern storage provider |
| `packages/workspace-sync/src/project-paths.ts` | Project dir layout |
| `packages/database/prisma/schema.prisma` | Migration target |
| `apps/api/src/features/projects/chat/` | Module mở rộng |
| `openclaw-architecture.md` §4.2 | Gateway media endpoint |
| `openclaw/src/gateway/chat-attachments.ts` | Parse inbound attachments, offload `media/inbound/` |
| `openclaw/src/gateway/server-methods/chat.ts` | `chat.send` handler, `prestageMediaPathOffloads` |
| `openclaw/src/gateway/managed-image-attachments.ts` | Outgoing images, `/api/chat/media/outgoing/…` |
| `openclaw/src/media/store.ts` | `saveMediaBuffer`, naming `{name}---{uuid}.ext` |
| `openclaw/src/gateway/protocol/schema/logs-chat.ts` | `ChatSendParamsSchema` |
| `deploy/scripts/gateway-entrypoint.sh` | `OPENCLAW_STATE_DIR` = project dir |

---

*Tạo: 2026-06-05 — cập nhật: 2026-06-08 (đối chiếu openclaw/openclaw, hướng D, rủi ro sandbox).*
