...
/** Files under OPENCLAW_DATA_ROOT — default storage for `apps/api` (§1.13). */
@Injectable()
export class DiskChatAttachmentStorage implements ChatAttachmentStorage {
  ...
  async save(
    input: ChatAttachmentSaveInput,
  ): Promise<ChatAttachmentSaveResult> {
    const ext = extensionForChatMime(input.mimeType);
    const relativePath = chatAttachmentRelativePath(
      input.kind,
      input.attachmentId,
      ext,
    );
    const dataDir = resolveProjectDataDir(input.projectId, {
