/**
 * Built-in agent templates — static catalog bundled in the repo.
 *
 * Templates are always available without a database seed step.
 */
export type AgentTemplateCatalogEntry = {
  slug: string;
  name: string;
  description: string;
  avatar: string;
  ...
  bootstrapAgents: string;
  sortOrder: number;
};

export const AGENT_TEMPLATE_CATALOG: readonly AgentTemplateCatalogEntry[] = [
  {
    slug: 'empty',
    name: 'Custom Agent',
    description:
