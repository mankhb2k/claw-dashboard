...
function redactDeviceTokens(entry: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [role, value] of Object.entries(tokens as Record<string, unknown>)) {
...
export class NodesService {
  ...
  async list(userId: string, projectId: string): Promise<NodeListResponse> {
    const res = await this.gateway.call<NodeListResponse>(userId, projectId, 'node.list', {});
    return {
      ts: res.ts,
  ...
  async getPairing(userId: string, projectId: string): Promise<NodesPairingResponse> {
    const [devices, nodes] = await Promise.all([
      this.gateway.call<DevicePairingList>(userId, projectId, 'device.pair.list', {}),
      this.gateway.call<NodePairingList>(userId, projectId, 'node.pair.list', {}),
    ]);

    return {
      devices: {
        pending: Array.isArray(devices.pending) ? devices.pending : [],
        paired: (Array.isArray(devices.paired) ? devices.paired : []).map(redactDeviceTokens),
      },
      nodes: {
        pending: Array.isArray(nodes.pending) ? nodes.pending : [],
        paired: (Array.isArray(nodes.paired) ? nodes.paired : []).map(redactPairedNode),
      },
    };
  }
  ...
  async rejectDevicePairing(
    ...
    return this.gateway.call(userId, projectId, 'device.pair.reject', { requestId });
  }
  ...
  async rejectNodePairing(
    ...
    return this.gateway.call(userId, projectId, 'node.pair.reject', { requestId });
  }
