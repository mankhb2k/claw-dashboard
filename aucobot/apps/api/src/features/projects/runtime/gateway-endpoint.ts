...
import { GatewayConfigError, isOssRuntime, type GatewayEndpoint } from '@aucobot/runtime-contracts';
import {
  httpBaseToWsBase,
...
export function resolveGatewayEndpoint(project?: ProjectGatewayPick): GatewayEndpoint {
  try {
    if (isOssRuntime()) {
