...
const SECTION_ITEMS: TabItem[] = [
  ...
  { value: 'schedules', label: 'Schedules', href: '#', badge: 1, badgeTone: 'danger' },
...
const PANEL_ITEMS_TEXT_ONLY: TabItem[] = PANEL_ITEMS.map(({ value, label }) => ({
  value,
  label,
}))
...
function TabsPlayground({
  ...
  const items = useMemo(
    () => (variant === 'section' ? SECTION_ITEMS : withIcons ? PANEL_ITEMS : PANEL_ITEMS_TEXT_ONLY),
    [variant, withIcons],
  )
  ...
  return (
    <Tabs
      ...
      aria-label={variant === 'section' ? 'Agent section' : 'Agent form sections'}
    />
  )
}
...
export const BotAgentSection: Story = {
  render: () => (
    <div>
      <DemoLabel>Bot Agent section nav (variant=&quot;section&quot;, showIndicator)</DemoLabel>
...
export const AgentIdPanel: Story = {
  render: () => (
    <div>
      <DemoLabel>Agent ID panel tabs (variant=&quot;panel&quot;, showIndicator=false)</DemoLabel>
      <TabsPlayground variant="panel" showIndicator={false} withIcons initialValue="identity" />
    </div>
  ),
}
...
export const PanelTextOnly: Story = {
  render: () => (
    <div>
      ...
      <TabsPlayground variant="panel" showIndicator={false} withIcons={false} initialValue="identity" />
    </div>
  ),
}
...
export const SectionNoIndicator: Story = {
  render: () => (
    <div>
      ...
      <TabsPlayground variant="section" showIndicator={false} initialValue="collaboration" />
    </div>
  ),
}
...
export const AllVariants: Story = {
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', maxWidth: 720 }}>
      <div>
        <DemoLabel>With indicator + icons (panel)</DemoLabel>
        <TabsPlayground variant="panel" showIndicator withIcons initialValue="capabilities" />
      </div>
      <div>
      ...
      <div>
        ...
        <TabsPlayground variant="section" showIndicator initialValue="schedules" />
      </div>
      <div>
        <DemoLabel>Section — text only, no indicator</DemoLabel>
        <TabsPlayground variant="section" showIndicator={false} initialValue="heartbeat" />
      </div>
    </div>
  ),
}
