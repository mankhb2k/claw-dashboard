...
> **Baseline hiện tại:** `pnpm --filter @aucobot/web lint` → **0 errors / 33 warnings**  
...
Sau batch fix: `pnpm --filter @aucobot/web lint` + `typecheck` + smoke chat composer.
...
- [ ] `pnpm --filter @aucobot/web lint` — còn ≤ 1 warning (chỉ `aria-expanded`)
- [ ] `pnpm --filter @aucobot/web typecheck` — pass
...
3. CI Web đã chạy `pnpm --filter @aucobot/web lint` — sẽ chặn merge nếu ai phá thứ tự import
...
```bash
cd aucobot
pnpm --filter @aucobot/web lint        # expect: 0 problems
