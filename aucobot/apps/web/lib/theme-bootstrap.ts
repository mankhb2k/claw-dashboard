...
export function getThemeBootstrapInlineScript(): string {
  ...
  return `try{var K=${lsKey};var C=${cookieName};var r=localStorage.getItem(K);if(!r)return;var j=JSON.parse(r);var t=j&&j.state&&j.state.theme;if(t==="dark"){document.documentElement.setAttribute("data-theme","dark");document.cookie=C+"=dark;path=/;max-age=${maxAge};SameSite=Lax";}else{document.documentElement.removeAttribute("data-theme");document.cookie=C+"=light;path=/;max-age=${maxAge};SameSite=Lax";}}catch(e){}`
}
