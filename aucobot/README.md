...
![Bot Agent configuration — customize agents, skills, tools, and workspace per slug](./apps/web/public/tab-bot-agent.png)
...
- **Agents & skills** — define bot personalities, tools, and skills per project; no markdown surgery required
...
![Live Chat — talk to agents in the browser, pick models per session, and stream replies from the gateway](./apps/web/public/tab-chat.png)
...
Open **http://localhost:8386** → sign in with **`admin` / `admin123`** → **create your first project** → connect a channel or connector from the dashboard. No OpenClaw CLI needed.
...
cd aucobot
pnpm install
cp .env.example .env          # single env file at repo root (API + Web)
...
**Local dev:** `aucobot/.env` (copy from `.env.example`)  
