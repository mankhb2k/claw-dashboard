...
import type { ToolDefinition, ToolResult } from '@aucobot/mcp-core';
import { errorResponse } from '@aucobot/mcp-core';
import type { DriveToolContext } from './tool-context.js';
import {
  escapeDriveQuery,
...
import {
  downloadDriveFileInline,
  formatInlineDownloadResult,
...
const UploadFileSchema = z
  .object({
    contentBase64: z.string().min(1, 'Base64 file content is required').optional(),
    name: z.string().optional(),
    parentFolderId: z.string().optional(),
  ...
  .superRefine((data, ctx) => {
    if (!data.contentBase64) {
      ctx.addIssue({
        ...
        message: 'contentBase64 is required for hosted upload',
      });
    }
  });
...
const DownloadFileSchema = z.object({
  ...
  exportMimeType: z
    .string()
    .optional()
    .describe('For Google Workspace files: MIME type to export as'),
  returnInline: z
    .boolean()
    .optional()
    .default(true)
    .describe('Return file content inline (default true on hosted MCP)'),
});
...
const UploadPdfWithSplitSchema = z
  .object({
    contentBase64: z.string().min(1, 'Base64 PDF content is required').optional(),
  ...
  .superRefine((data, ctx) => {
    if (!data.contentBase64) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'contentBase64 is required for hosted PDF upload',
      });
    }
  });

async function splitPdfIntoChunkFiles(pdfBytes: Buffer, maxPagesPerChunk: number): Promise<{ tempDir: string; files: string[] }> {
...
export const toolDefinitions: ToolDefinition[] = [
  ...
  {
    name: "uploadFile",
    description: "Upload a file to Google Drive using base64-encoded content (hosted MCP). Supports images, audio, video, PDF, Office docs, etc.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        mimeType: { type: "string", description: "MIME type (auto-detected from name if omitted)" },
        convertToGoogleFormat: { type: "boolean", description: "Convert to Google Workspace format (.docx→Doc, etc.)" }
      },
  ...
  {
    name: "downloadFile",
    description: "Download a Google Drive file and return content inline. Google Workspace files are exported (Docs→text, Sheets→csv by default).",
    inputSchema: {
      type: "object",
      properties: {
        fileId: { type: "string", description: "Google Drive file ID" },
        exportMimeType: { type: "string", description: "Export MIME for Workspace files" },
        returnInline: { type: "boolean", description: "Return content inline (default true)" }
      },
      required: ["fileId"]
    }
  },
  ...
  },
  {
    ...
    description: "Upload a PDF from base64 content and optionally split into chunked parts",
    inputSchema: {
      type: "object",
      properties: {
        contentBase64: { type: "string", description: "Base64-encoded PDF content" },
        split: { type: "boolean", description: "Enable split mode" },
        maxPagesPerChunk: { type: "number", description: "Max pages per chunk (default 25)" },
        parentFolderId: { type: "string", description: "Destination folder ID" },
...
export async function handleTool(
  ...
  switch (toolName) {
    ...
    case "uploadFile": {
      ...
      ctx.log('Uploading file', { name: uploadName, mimeType: detectedMime, convertToGoogle: !!targetMimeType, size: fileBuffer.length });
      ...
      return {
        content: [{
          ...
          text: [
            ...
            `ID: ${file.data?.id || 'unknown'}`,
            `Size: ${file.data?.size || fileBuffer.length} bytes`,
    ...
    case "downloadFile": {
      ...
      if (data.returnInline === false) {
        return errorResponse('Hosted MCP only supports inline download (returnInline=true).');
      }

      const downloadResult = await downloadDriveFileInline(
        ctx.getDrive(),
        { fileId: data.fileId, exportMimeType: data.exportMimeType },
        ctx.log,
      );
      ...
      return {
        content: [{
          type: 'text',
          text: formatInlineDownloadResult(downloadResult),
        }],
        isError: false,
      };
    }
    ...
    case "uploadPdfWithSplit": {
      ...
      const pdfBytes = Buffer.from(data.contentBase64!, 'base64');
      const parentId = await ctx.resolveFolderId(data.parentFolderId);

      if (!data.split) {
        const fileName = data.namePrefix ? `${data.namePrefix}.pdf` : 'upload.pdf';
      ...
      const baseName = data.namePrefix || 'upload';

      let tempDir: string | undefined;
      try {
        const splitResult = await splitPdfIntoChunkFiles(pdfBytes, maxPagesPerChunk);
        ...
        for (let i = 0; i < splitResult.files.length; i++) {
          ...
          const uploaded = await ctx.getDrive().files.create({
            ...
            media: { mimeType: 'application/pdf', body: Readable.from(partBytes) },
            fields: 'id,name,webViewLink',
            supportsAllDrives: true,
          });
...
export const driveToolSchemas: Record<string, z.ZodTypeAny> = {
  search: SearchSchema,
  createTextFile: CreateTextFileSchema,
  updateTextFile: UpdateTextFileSchema,
  createFolder: CreateFolderSchema,
  listFolder: ListFolderSchema,
  listSharedDrives: ListSharedDrivesSchema,
  deleteItem: DeleteItemSchema,
  renameItem: RenameItemSchema,
  moveItem: MoveItemSchema,
  ...
  removePermission: RemovePermissionSchema,
  shareFile: ShareFileSchema,
  convertPdfToGoogleDoc: ConvertPdfToGoogleDocSchema,
  bulkConvertFolderPdfs: BulkConvertFolderPdfsSchema,
  uploadPdfWithSplit: UploadPdfWithSplitSchema,
  getRevisions: GetRevisionsSchema,
  restoreRevision: RestoreRevisionSchema,
  createShortcut: CreateShortcutSchema,
  lockFile: LockFileSchema,
  unlockFile: UnlockFileSchema,
};
