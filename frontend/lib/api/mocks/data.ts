...
/** Stable fake gateway tokens for mock interceptor (lazy-filled). */
...
// Test helpers for mock env flows
export function seedMockProjectEnv(projectId: string, env: Record<string, string>) {
  mockProjectEnv.set(projectId, new Map(Object.entries(env)))
}

export function getMockProjectEnv(projectId: string): Record<string, string> {
  return Object.fromEntries(mockProjectEnv.get(projectId)?.entries() ?? [])
}

export function resetMockProjectEnv(projectId?: string) {
  if (projectId) {
    mockProjectEnv.delete(projectId)
    return
  }
