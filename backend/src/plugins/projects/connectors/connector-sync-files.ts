...
/** Ghi file credential Google Drive MCP vào volume project. */
export async function writeGoogleDriveCredentialFiles(
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error('Thiếu client_id, client_secret hoặc refresh_token cho Google Drive');
  }
...
export function buildMcpServerEntry(
  ...
  if (def.slug === 'google-drive') {
    const oauth = containerPaths.oauthPath ?? `${CONTAINER_STATE_DIR}/connectors/google-drive/gcp-oauth.keys.json`;
    const creds =
      containerPaths.credentialsPath ?? `${CONTAINER_STATE_DIR}/connectors/google-drive/credentials.json`;
    return {
      command: 'npx',
  ...
  if (def.slug === 'google-calendar') {
    ...
    const refreshToken = secrets.refresh_token?.trim();
    if (!clientId || !clientSecret || !refreshToken) {
      throw new Error('Thiếu OAuth credentials cho Google Calendar');
    }
    return {
      command: 'npx',
