...
import { ConnectorConnectionStatus } from '@prisma/client';
...
export class ProjectConnectorsService {
  ...
  async create(
    ...
    const def = resolveConnector(input.connectorSlug);
    if (!def) {
    ...
    const existing = await this.prisma.projectConnector.findUnique({
      where: { projectId_connectorSlug: { projectId, connectorSlug: def.slug } },
    });
    if (existing) {
  ...
  async test(projectId: string, connectorId: string): Promise<{ ok: boolean; message?: string }> {
    ...
    try {
      ...
      await this.markTestResult(row.id, true, null);
      await this.syncOpenClawConfig(projectId);
  ...
  async startOAuth(userId: string, projectId: string, connectorSlug: string): Promise<{ url: string }> {
    const def = resolveConnector(connectorSlug);
    if (!def?.oauthScopes?.length) {
    ...
    const state = await this.jwt.signAsync(
      { purpose: 'connector_oauth', sub: userId, projectId, connectorSlug: def.slug },
      { expiresIn: '15m' },
    );
  ...
  async handleOAuthCallback(code: string, state: string): Promise<{ redirectUrl: string }> {
    ...
    const def = resolveConnector(payload.connectorSlug);
    if (!def) {
    ...
    const secretPayload = [
      ...
      { key: 'client_secret', value: cfg.clientSecret },
    ...
    const redirectUrl = `${frontend}/dashboard/connect/${def.slug}?connected=1`;
    return { redirectUrl };
  }
  ...
  private toDto(
    ...
    return {
      ...
      connectorDefinitionId: def?.id ?? row.connectorSlug,
      connectorSlug: row.connectorSlug,
      connectorName: def?.displayName ?? row.displayName,
      connectorKind: def?.kind ?? 'OAUTH',
      ...
      definition: def
        ? { description: def.description, status: def.status, configSchema: null }
        : undefined,
    };
  }
}
