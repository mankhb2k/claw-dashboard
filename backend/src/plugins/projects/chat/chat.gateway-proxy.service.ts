...
import { isAllowedChatRpc } from './chat-rpc-whitelist';
import { openGatewayUpstream } from './gateway-upstream';
...
export class ChatGatewayProxyService {
  ...
  async bridge(params: {
    ...
    const { client, projectId, hostPort, gatewayToken } = params;
    let upstream: UpstreamSocket;
    const projectDataDir = this.workspace.resolveProjectDataDir(projectId);

    try {
      upstream = await openGatewayUpstream(hostPort, gatewayToken, projectDataDir);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'GATEWAY_UNREACHABLE';
      this.log.warn(`Upstream connect failed (port=${hostPort}): ${message}`);
      sendJson(client, {
        type: 'event',
        event: 'proxy.error',
