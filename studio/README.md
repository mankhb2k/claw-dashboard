...
```text
claw-dashboard/
└── studio/          ← Claw Dashboard monorepo (apps, packages, deploy)
```
...
**Local dev:** `studio/apps/.env` (copy from `apps/.env.example`)  
**Docker:** `studio/deploy/.env` (copy from `deploy/.env.example`)
