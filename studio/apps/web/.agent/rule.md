# Nguyên tắc Phát triển Frontend — Claw Dashboard Web

> Áp dụng cho `studio/apps/web/`. Mọi code (kể cả AI) phải tuân thủ.  
> Monorepo: đọc thêm `studio/AGENTS.md` khi task chạm nhiều app/package.  
...
- **schemas:** web-first Zod; dùng `@claw-dashboard/shared` khi type đã có — không duplicate.
...
- **`studio/apps` storage = local** (avatar PG, chat disk) — không phụ thuộc `RUNTIME_MODE` (API: `apps/api/.agent/rule.md` §1.13).
- **`studio/apps` là OSS thuần** — không còn `NEXT_PUBLIC_RUNTIME_MODE` hay UI cloud (spawn container, subdomain URL).

### 6.2 Trong phạm vi

`studio/apps/web/` — `app/`, `components/`, `hooks/`, `utils/`, `lib/`, `schemas/`, `stores/`, `public/`.
...
```bash
pnpm --filter @claw-dashboard/web test
```
...
- Dev: `pnpm --filter @claw-dashboard/web storybook` (port 6006).
- Build static: `pnpm --filter @claw-dashboard/web build-storybook`.
...
| Lint | `pnpm --filter @claw-dashboard/web lint` | **0 error** |
| Typecheck | `pnpm --filter @claw-dashboard/web typecheck` | **0 error** |
| Build | `pnpm turbo run build --filter=@claw-dashboard/web` | Thành công |
...
```bash
pnpm --filter @claw-dashboard/web lint
...
| Lệnh | Mục đích |
|------|----------|
| `pnpm --filter @claw-dashboard/web dev` | Dev server port **8386** |
| `pnpm --filter @claw-dashboard/web build` | `next build` production |
| `pnpm --filter @claw-dashboard/web start` | Serve build port 8386 |

### 8.4 Đóng gói

- Output: `.next/` (gitignore).
- `transpilePackages`: workspace deps (`@claw-dashboard/workspace-sync`, …).
...
### 9.3 Luật `error` — kiến trúc Claw Dashboard
