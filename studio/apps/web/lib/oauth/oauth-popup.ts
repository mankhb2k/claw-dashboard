export type OAuthPopupMessage =
  | { type: 'claw-dashboard-oauth-complete'; slug?: string }
  | { type: 'claw-dashboard-oauth-error'; error: string };

const POPUP_NAME = 'claw-dashboard-oauth';
...
export function waitForOAuthPopupResult(
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (data.type === 'claw-dashboard-oauth-complete') {
    ...
    if (data.type === 'claw-dashboard-oauth-error') {
      cleanup();
