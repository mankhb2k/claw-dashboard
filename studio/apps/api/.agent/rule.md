...
> Áp dụng cho `studio/apps/api/`. Mọi code do AI tạo phải tuân thủ.  
> Monorepo: đọc thêm `studio/AGENTS.md`, [`workflow.md`](../../../../workflow.md) (repo root; `studio/docs/workflow.md` chỉ redirect), `studio/docs/monorepoplan.md` khi task chạm runtime/sync/cloud.  
...
- Dashboard dùng **JWT** (access + refresh) — logic token/cookie trong `@claw-dashboard/control-plane-core`.
...
- API key provider, channel token, connector OAuth: **mã hóa at-rest** qua `encryptSecret` / `decryptSecret` (`@claw-dashboard/control-plane-core`).
...
### 1.11 OSS attack surface

| Quy tắc | Ghi chú |
| ------- | ------- |
| **Cấm** mount `docker.sock` vào `api` | Gateway chạy container riêng; API chỉ sync config |
| Một gateway chung `:18789` | Self-host disk layout `data/{projectId}/` |
| Runtime **OSS cố định** | Không còn `RUNTIME_MODE=cloud` hay UI cloud trong repo |

- Code spawn Docker per-project / billing / multi-tenant **không** thêm vào OSS path.

### 1.13 Self-host — lưu trữ local (`studio/apps`)

> Repo này là **OSS thuần**: storage local + shared gateway `:18789`.

| Thành phần | `studio/apps` |
| ---------- | ------------- |
| **Avatar** | PostgreSQL (`PostgresAvatarStorage`) |
| **Chat attachment** | Disk (`DiskChatAttachmentStorage`) |
| **Runtime gateway** | Shared `:18789`, sync `openclaw.json` qua API |

**Quy tắc code:**

- Provider **không** branch runtime mode — luôn `PostgresAvatarStorage` + `DiskChatAttachmentStorage`.
- **`apps/api` cấm import cloud-only packages** — ESLint `no-restricted-imports` **error** nếu thêm sau này.
- `cleanup:chat-attachments` chỉ xóa file local.
- `isOssRuntime()` luôn `true` — giữ helper cho code path gateway OSS, không dùng chọn storage backend.
...
Controller  →  Service (apps/api)  →  Prisma / WorkspaceService
                    ↓
              @claw-dashboard/control-plane-core | workspace-sync | runtime-oss
```
...
### 2.5 Thư viện chung `@claw-dashboard/*` — bảng tra team
...
| --------------------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------ |
| **`@claw-dashboard/shared`**             | `**/lib/**`, `**/dto/**`, controller, **web**            | Enum/const dùng chéo FE+BE (`UsageSource`, `ConnectorKind`, model catalog types, channel defs, auth cookie names) | Prisma, Nest, fs, crypto |
| **`@claw-dashboard/database`**           | `**/services/**`, `core/database/**`, mapper gần service | `PrismaClient`, `Prisma` types, enum **runtime ghi DB**                                                           | Business logic           |
| **`@claw-dashboard/control-plane-core`** | service, guard, `main.ts`                                | JWT, `encryptSecret`/`decryptSecret`, gateway upstream, chat whitelist, self-host seed                            | `@nestjs/*`, Prisma      |
| **`@claw-dashboard/workspace-sync`**     | service workspace/agent/skill                            | `openclaw.json` merge, compile agent, fs layout, collaboration helpers                                            | Nest, HTTP               |
| **`@claw-dashboard/runtime-contracts`**  | service, provider factory, `main.ts`                     | Interfaces (`AvatarStorage`, `ChatAttachmentStorage`), limits, `isOssRuntime()`                                   | Implementation           |
| **`@claw-dashboard/runtime-oss`**        | service runtime/gateway (OSS branch)                     | Static gateway URL, health poll, `StaticGatewayProvisioner`                                                       | Cloud/Docker             |
| **`@claw-dashboard/clawhub-client`**     | service skill/ClawHub                                    | ClawHub HTTP client, parse response                                                                               | Nest                     |

> **`@claw-dashboard-cloud/*`:** **không** dùng trong `studio/apps` — chỉ repo `../cloud/` (§1.13).

#### Quy tắc import theo layer (ESLint `no-restricted-imports` → **error**)

| Layer path                        | Được import                                                                                          | Cấm (runtime)                                                                                        |
| --------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `**/dto/**`                       | `@claw-dashboard/shared`, `class-validator`, `nestjs` (không `@Injectable`)                                 | `prisma.service`, `@claw-dashboard/database` runtime                                                        |
| `**/lib/**`                       | `@claw-dashboard/shared`, `@claw-dashboard/control-plane-core` (crypto thuần), `@claw-dashboard/runtime-contracts` (type) | `prisma.service`, `@claw-dashboard/database` **value** — dùng `@claw-dashboard/shared` cho enum (`UsageSource`, …) |
| `**/*.controller.ts`              | service, dto, `@claw-dashboard/shared`                                                                      | `prisma.service`, `@claw-dashboard/database` — delegate service                                             |
| `**/services/**`                  | Mọi package trên + `@claw-dashboard/database`                                                               | —                                                                                                    |
| `src/core/**` _(trừ `database/`)_ | packages, `core/*`                                                                                   | `features/**` _(type-only OK)_                                                                       |

**Enum Prisma mirror:** thêm const/object vào `@claw-dashboard/shared` (vd. `UsageSource`, `UsageStatus`) — **không** import runtime `@claw-dashboard/database` trong `lib/`. Service ghi DB vẫn dùng enum Prisma khi cần.
...
Docker per-project spawn → sibling repo **`../cloud/packages/fleet`** (`@claw-dashboard-cloud/fleet`). **Không** có `docker/` trong OSS `apps/api`.
...
Catalog types + `resolveEffectiveAgentModel` → **`@claw-dashboard/shared`** (`packages/shared/src/models/`). Loader `loadProjectModelCatalog` vẫn ở `chat/lib/project-model-catalog.ts` (Prisma/Workspace).
...
`OPENAI_CHAT_MODELS`, `GEMINI_CHAT_MODELS` + `*.types.ts` → **`@claw-dashboard/shared`**. Root strict: chỉ module + controller.
...
`OPENCLAW_CHANNEL_DEFS` (skeleton id/docs/kind) + access validation helpers → **`@claw-dashboard/shared`**. `CHANNEL_REGISTRY` (API) chỉ gồm adapter đã implement. `ChannelsModule` exports `ChannelsService`; `ProjectsModule` re-export `ChannelsModule`.
...
`ConnectorKind` → **`@claw-dashboard/shared`**. `CONNECTOR_REGISTRY` (API) chỉ gồm adapter đã implement. `ConnectorsModule` exports `ProjectConnectorsService`; `ProjectsModule` re-export `ConnectorsModule`.
...
> **Chính sách cố định:** studio có **một** đường đưa foundation provider API key xuống OpenClaw gateway — block `env` trong `{projectId}/openclaw.json`. **Không** dùng cơ chế song song.
...
UI Save provider key
  ...
  → mergeProviderKeysIntoConfig() (@claw-dashboard/workspace-sync)
...
| Cách                                                                                     | Lý do cấm                                                                  |
| ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `agents/{agentId}/agent/auth-profiles.json`                                              | Trùng nguồn sự thật; OpenClaw CLI/onboard path — **không** sync từ studio |
...
**Lưu ý OpenClaw worker:** `resolveApiKeyForProvider` vẫn _có thể_ đọc auth profile nếu file tồn tại (thứ tự profile-first). studio **không tạo/không ghi** `auth-profiles.json` — chỉ dựa vào `openclaw.json` `env` sau khi gateway load config.
...
- Runner: **Jest** (`pnpm --filter @claw-dashboard/api test`)
...
- Import workspace packages: `@claw-dashboard/control-plane-core`, `@claw-dashboard/workspace-sync`, `@claw-dashboard/database`, `@claw-dashboard/shared`
...
- Dùng `@claw-dashboard/shared` / package schema có sẵn — **không** duplicate (giống web §3.G).
- Dependency: thêm `zod` vào `@claw-dashboard/api` khi viết schema mới (`pnpm --filter @claw-dashboard/api add zod` — align version với `apps/web`).
...
```text
studio/apps/api/src/
studio/packages/control-plane-core/   # nếu extract logic thuần
studio/packages/workspace-sync/     # nếu chạm sync file
studio/packages/database/           # nếu chạm schema
```
...
```bash
pnpm --filter @claw-dashboard/api lint          # local (--fix)
pnpm --filter @claw-dashboard/api lint:ci       # CI parity (no --fix)
pnpm --filter @claw-dashboard/runtime-contracts build   # nếu chạm contracts
pnpm --filter @claw-dashboard/runtime-oss build         # nếu chạm runtime-oss
pnpm --filter @claw-dashboard/control-plane-core build  # nếu chạm core package
pnpm --filter @claw-dashboard/workspace-sync build      # nếu chạm sync
pnpm --filter @claw-dashboard/api build
pnpm --filter @claw-dashboard/api test                  # khi chạm service/spec
```
...
```text
Task: [một câu]
Scope: studio/apps/api/src/features/projects/<domain>/
...
| `studio/AGENTS.md`                         | Monorepo commands, package map           |
| `studio/docs/monorepoplan.md`              | Folder roles, Phase extract              |
...
| `import/order` | Nhóm + `@claw-dashboard/**` internal + alphabetize + `type` last |
...
| `**/lib/**` | `prisma.service`, `@claw-dashboard/database` value | `@claw-dashboard/shared` enum · `import type` OK |
...
| `src/**` | `@claw-dashboard-cloud/*` | Self-host only — §1.13 |
...
"eslint.workingDirectories": [
  { "directory": "studio/apps/web", "changeProcessCWD": true },
  { "directory": "studio/apps/api", "changeProcessCWD": true }
]
```
...
- Local: `pnpm --filter @claw-dashboard/api lint` · CI: `lint:ci` (read-only).
- [`.github/workflows/api-ci.yml`](../../.github/workflows/api-ci.yml): **lint 0 error** + **build**; không phụ thuộc `@claw-dashboard-cloud/*` (§1.13).
...
- [ ] `pnpm --filter @claw-dashboard/api lint` pass (0 error theo §7.4)
- [ ] `pnpm --filter @claw-dashboard/api build` pass
