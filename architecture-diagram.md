...
graph TB
    ...
    subgraph Railway["Railway (Control Plane)"]
        ...
        Redis["📦 Redis Managed<br/>Queue: jobs:vps-1<br/>Queue: jobs:vps-2<br/>Queue: heavy-tasks"]
        CFAPI["☁️ Cloudflare API<br/>Create DNS on spawn<br/>Delete DNS on destroy"]
    end

    subgraph VPS1["VPS Worker #1 (Contabo SG)"]
        W1["🟢 vps-worker<br/>VPS_NODE_ID=vps-1<br/>Subscribe: jobs:vps-1"]
        T1["🔄 Traefik<br/>*.openclaw.ai"]
        C1a["📱 abc container"]
        C1b["📱 def container"]
    end

    subgraph VPS2["VPS Worker #2 (Contabo SG)"]
        W2["🟢 vps-worker<br/>VPS_NODE_ID=vps-2<br/>Subscribe: jobs:vps-2"]
        T2["🔄 Traefik<br/>*.openclaw.ai"]
        C2a["📱 xyz container"]
        C2b["📱 ghi container"]
    end
    ...
    User -->|xyz.openclaw.ai| CF
    CF -->|VPS-2 IP| T2
    T2 --> C2a

    API -->|Enqueue jobs:vps-1| Redis
    API -->|Enqueue jobs:vps-2| Redis
    API -->|Create A record| CFAPI
    CFAPI -.->|abc → VPS-1 IP| CF
...
## 16. DNS Management Flow (Phase 2)
...
sequenceDiagram
    ...
    participant VPS as 🟢 VPS Worker

    Note over API,VPS: === PROJECT CREATE ===

    User->>API: POST /api/projects
    API->>DB: SELECT vps_nodes ORDER BY active_count ASC LIMIT 1
    DB-->>API: vps-1 (ip: 103.x.x.x, active: 12)

    API->>API: Generate subdomain: "mankhb-a3x7"
    API->>DB: Check UNIQUE subdomain

    API->>CF: POST /zones/{zone}/dns_records<br/>{ type:A, name:"mankhb-a3x7", content:"103.x.x.x", proxied:true }
    CF-->>API: { id: "cf-record-abc123" }

    API->>DB: INSERT project { subdomain, vps_id:"vps-1", dns_record_id:"cf-record-abc123", status:"creating" }
    API->>Redis: Enqueue { queue:"jobs:vps-1", type:"spawn", projectId }

    VPS->>Redis: Pull spawn job
    VPS->>VPS: docker run + Traefik labels
    VPS->>API: PUT /api/internal/status { status:"running" }
    API->>DB: UPDATE vps_nodes SET active_count = active_count + 1

    Note over API,VPS: === PROJECT DESTROY ===

    User->>API: DELETE /api/projects/:id
    API->>Redis: Enqueue { queue:"jobs:vps-1", type:"destroy", projectId }
    VPS->>VPS: docker stop + docker rm + rm -rf /data/users/{id}
    VPS->>API: PUT /api/internal/status { status:"destroyed" }
    API->>CF: DELETE /zones/{zone}/dns_records/cf-record-abc123
    API->>DB: UPDATE vps_nodes SET active_count = active_count - 1
```

---

## 17. High Scale Architecture (Phase 3 — > 10,000 users)
...
graph TB
    ...
    subgraph CF["Cloudflare Edge"]
        Worker["⚡ CF Workers<br/>extract subdomain<br/>lookup KV store<br/>&lt;1ms routing"]
        KV["🗃️ CF KV Store<br/>abc → vps-1<br/>xyz → vps-3<br/>..."]
        Tunnel1["🔒 CF Tunnel vps-1"]
        Tunnel2["🔒 CF Tunnel vps-2"]
    ...
    subgraph Redis_VPS["Redis Sentinel VPS (Dedicated)"]
        RSentinel["🔴 Redis Sentinel<br/>1 Master + 2 Replicas<br/>~$20/mo<br/>~1ms latency"]
    end

    subgraph VPS1["VPS Worker #1"]
        T1["🔄 Traefik"] --> C1["📱 Containers"]
        W1["🟢 vps-worker<br/>Subscribe: jobs:vps-1"]
    end

    subgraph VPS2["VPS Worker #2"]
        T2["🔄 Traefik"] --> C2["📱 Containers"]
        W2["🟢 vps-worker<br/>Subscribe: jobs:vps-2"]
    end

    subgraph VPS3["VPS Worker #3"]
        T3["🔄 Traefik"] --> C3["📱 Containers"]
        W3["🟢 vps-worker<br/>Subscribe: jobs:vps-3"]
    end

    User -->|*.openclaw.ai| Worker
    Worker -->|KV lookup| KV
    KV -->|abc = vps-1| Tunnel1
    KV -->|xyz = vps-3| Tunnel3
...
erDiagram
    ...
    vps_nodes ||--o{ projects : hosts
    plans ||--o{ users : subscribes
    projects ||--o{ cron_schedules : has
```

---

Generated by: claude-code | Last updated: 2026-04-23
